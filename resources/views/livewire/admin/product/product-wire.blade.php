                    ...
                    <div class="kt-portlet__head-wrapper">
                        <div class="kt-portlet__head-actions">
                            <div class="dropdown dropdown-inline">
                                <a wire:navigate href="{{ route('admin.product.create')}}" class="btn btn-brand btn-elevate btn-icon-sm">
        ...
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="kt_table_1">
                    ...
                    <tr>
                        <th class="w-auto">#</th>
                        <th>{{__('validation.attributes.vendors_id')}}</th>
                        <th>{{__('validation.attributes.shop_name')}}</th>
                        <th>{{__('validation.attributes.vendors_name')}}</th>
                        <th>{{__('validation.attributes.vendors_l_name')}}</th>
                        <th>{{__('validation.attributes.title')}}</th>
                        <th>{{__('validation.attributes.price')}}</th>
                        <th>{{__('validation.attributes.quantity')}}</th>
                        <th>{{__('validation.attributes.status')}}</th>
                        <th>{{__('validation.attributes.categories')}}</th>
                    ...
                    @foreach($products as $p)
                        <tr>
                            ...
                            <td>
                                @switch($p->status)
                                    @case(0)
                                        <span class="badge badge-secondary">{{__('validation.attributes.inactive')}}</span>
                                        @break
                                    @case(1)
                                        <span class="badge badge-success">{{__('validation.attributes.active')}}</span>
                                        @break
                                @endswitch
                            </td>
                            <td>
                            ...
                            <td>{{verta($p->created_at)->format('Y/m/d')}}</td>
                            <td class="text-center d-flex justify-content-between">
                                <a wire:click="deleteproduct({{$p->id}})" class="text-danger btn btn-bolder border-danger btn-sm d-flex justify-content-center align-items-center" style="width:35px; height:35px;">
                                    <i class="fa fa-trash"></i>
                                </a>
                                <a wire:navigate href="{{route('admin.product.edit',['product'=>$p->id])}}" class="text-info btn btn-bolder border-info btn-sm d-flex justify-content-center align-items-center" style="width:35px; height:35px; margin-left:5px;">
                ...
                <div class="mt-2">
                    @if(app()->getLocale() === 'fa')
                        نمایش {{ $products->firstItem() }} تا {{ $products->lastItem() }} از {{ $products->total() }} نتیجه
                    @endif
                </div>
                {{$products->links()}}
                <!--end: Datatable -->
