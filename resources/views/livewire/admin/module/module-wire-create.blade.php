<div>
    <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor">

        <!-- begin:: Content Head -->
        <div class="kt-subheader   kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                <h3 class="kt-subheader__title">
                    {{__('validation.attributes.create_module')}}
                </h3>
                <span class="kt-subheader__separator kt-subheader__separator--v"></span>
                <div class="kt-input-icon kt-input-icon--right kt-subheader__search kt-hidden">
                    <input type="text" class="form-control" placeholder="Search order..." id="generalSearch">
                    <span class="kt-input-icon__icon kt-input-icon__icon--right">
										<span><i class="flaticon2-search-1"></i></span>
									</span>
                </div>
            </div>
            <div class="kt-subheader__toolbar">
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-wrapper">
                        <div class="kt-portlet__head-actions">
                            <div class="dropdown dropdown-inline">
                                <a wire:navigate href="{{ route('admin.module.index') }}" class="btn btn-brand btn-elevate btn-icon-sm">
                                    <i class="la la-plus"></i>
                                    {{__('validation.massages.show_the_list')}}
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- end:: Content Head -->
    </div>
    <div class="d-flex justify-content-center align-items-center">
{{--        @if($msg != '')--}}
{{--            <div class="alert alert-light alert-elevate w-25 @if($msg_status == 'success') alert-success @else alert-danger @endif">--}}
{{--                <i class="mx-3 fa @if($msg_status == 'success') fa-check-circle @else fa-times-circle @endif"></i>--}}
{{--                {{$msg}}--}}
{{--            </div>--}}
{{--        @endif--}}
    </div>
    <div class="kt-content kt-grid__item kt-grid__item--fluid" id="kt_content">
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="kt-portlet">
                    <div class="kt-portlet__head">
                        <div class="kt-portlet__head-label">
                            <h3 class="kt-portlet__head-title">
                                <i class="fa fa-plus text-primary mx-2"></i>
                                {{__('validation.attributes.create_module')}}
                                {{--                                @if(app()->getLocale() === 'fa')--}}
                                {{--                                    <span>زبان جاری: فارسی</span>--}}
                                {{--                                @else--}}
                                {{--                                    <span>Current Language: English</span>--}}
                                {{--                                @endif--}}
                            </h3>
                        </div>
                    </div>
                    <form class="kt-form" wire:submit.prevent="save">
                        <div class="kt-portlet__body">
                            <div class="form-group">
                                <label>{{__('validation.attributes.fa_title')}}</label>
                                <input autocomplete="off" wire:model="name" :key="$name" type="text" class="form-control" placeholder="{{__('validation.massages.enter_title')}}">
{{--                                @error('name')--}}
{{--                                <div class="alert alert-danger">--}}
{{--                                    <i class="mx-3 fa fa-exclamation-triangle"></i>--}}
{{--                                    {{$message}}--}}
{{--                                </div>--}}
{{--                                @enderror--}}
                            </div>
                            <div class="form-group">
                                <label for="exampleSelect1">{{__('validation.attributes.menu')}} {{__('validation.attributes.parent')}} </label>
                                <select autocomplete="off" wire:model="parent" :key="parent" class="form-control" id="exampleSelect1">
                                    <option value="">__انتخاب منوی والد __</option>
{{--                                    @foreach($menus as $m)--}}
{{--                                        <option value="{{$m->id}}">--}}
{{--                                            @if(app()->getLocale()==='fa')--}}
{{--                                                {{$m->title}}--}}
{{--                                            @else--}}
{{--                                                {{$m->title_en ?? $m->title}}--}}
{{--                                            @endif--}}
{{--                                        </option>--}}
{{--                                    @endforeach--}}
                                </select>

                            </div>
                            <div class="form-group form-group-marginless">
                                <label>{{__('validation.attributes.location')}} {{__('validation.attributes.menu')}}</label>
                                <div class="row">
                                    <div class="col-lg-4">
                                        <label class="kt-option">
                                            <span class="kt-option__control">
                                                <span class="kt-radio kt-radio--check-bold">
                                                    <input wire:model="location" type="radio" name="m_option_1" value="0" checked>
                                                    <span></span>
                                                </span>
                                            </span>
                                            <span class="kt-option__label">
                                                <span class="kt-option__head">
                                                    <span class="kt-option__title">
                                                        {{__('validation.attributes.header')}}
                                                    </span>
                                                </span>
                                                <span class="kt-option__body">
                                                    به عنوان منوی اصلی یا زیرمنو
                                                </span>
                                            </span>
                                        </label>
                                    </div>
                                    <div class="col-lg-4">
                                        <label class="kt-option">
                                            <span class="kt-option__control">
                                                <span class="kt-radio kt-radio--check-bold">
                                                    <input type="radio" wire:model="location" name="m_option_1" value="1">
                                                    <span></span>
                                                </span>
                                            </span>
                                            <span class="kt-option__label">
                                                <span class="kt-option__head">
                                                    <span class="kt-option__title">
                                                        {{__('validation.attributes.footer')}}
                                                    </span>
                                                </span>
                                                <span class="kt-option__body">
                                                    در فوتر به عنوان منو های دسترسی سریع
                                                </span>
                                            </span>
                                        </label>
                                    </div>
                                    <div class="col-lg-4">
                                        <label class="kt-option">
                                            <span class="kt-option__control">
                                                <span class="kt-radio kt-radio--check-bold">
                                                    <input wire:model="location" type="radio" name="m_option_1" value="3" checked>
                                                    <span></span>
                                                </span>
                                            </span>
                                            <span class="kt-option__label">
                                                <span class="kt-option__head">
                                                    <span class="kt-option__title">
                                                        {{__('validation.attributes.header')}} _ {{__('validation.attributes.footer')}}
                                                    </span>
                                                </span>
                                                <span class="kt-option__body">
                                                    قرار گیری در هدر و فوتر
                                                </span>
                                            </span>
                                        </label>
                                    </div>
                                </div>
{{--                                @error('location')--}}
{{--                                <div class="alert alert-danger">--}}
{{--                                    <i class="mx-3 fa fa-exclamation-triangle"></i>--}}
{{--                                    {{$message}}--}}
{{--                                </div>--}}
{{--                                @enderror--}}
                            </div>
                            <div class="form-group mt-2">
                                <label>{{__('validation.attributes.status')}}</label>
                                <div class="kt-radio-inline">
                                    <label class="kt-radio">
                                        <input autocomplete="off" value="1" wire:model="status" :key="status" type="radio" name="radio2"> {{__('validation.attributes.active')}}
                                        <span></span>
                                    </label>
                                    <label class="kt-radio">
                                        <input autocomplete="off" value="0" wire:model="status" :key="$name" type="radio" name="radio2"> {{__('validation.attributes.inactive')}}
                                        <span></span>
                                    </label>
{{--                                    @error('status')--}}
{{--                                    <div class="alert alert-danger">--}}
{{--                                        <i class="mx-3 fa fa-exclamation-triangle"></i>--}}
{{--                                        {{$message}}--}}
{{--                                    </div>--}}
{{--                                    @enderror--}}
                                </div>
                            </div>
                            <label class="kt-checkbox kt-checkbox--brand">
                                <input wire:model.live="translate_check" type="checkbox">{{__('validation.massages.add_translation')}}
                                <span></span>
                            </label>
{{--                            @if($translate_check)--}}
{{--                                <div class="form-group">--}}
{{--                                    <label>{{__('validation.attributes.en_title')}}</label>--}}
{{--                                    <input autocomplete="off" wire:model="translate_name" :key="translate_name" type="text" class="form-control" placeholder="{{__('validation.massages.enter_title')}}">--}}
{{--                                    @error('translate_name')--}}
{{--                                    <div class="alert alert-danger">--}}
{{--                                        <i class="mx-3 fa fa-exclamation-triangle"></i>--}}
{{--                                        {{$message}}--}}
{{--                                    </div>--}}
{{--                                    @enderror--}}
{{--                                </div>--}}
{{--                            @endif--}}
                        </div>
                        <div class="kt-portlet__foot">
                            <div class="kt-form__actions kt-form__actions--center">
                                <button type="submit"  class="btn btn-brand btn-elevate btn-icon-sm">
                                    <i class="la la-check-circle"></i>
                                    {{__('validation.attributes.submit')}}</button>
                                <button type="button" wire:click="clearForm" class="btn btn-secondary">{{__('validation.attributes.cancel')}}</button>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>
