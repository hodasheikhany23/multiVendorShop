    ...
    <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor">
        ...
        <div class="kt-subheader   kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                <h3 class="kt-subheader__title">{{__('validation.massages.list_menu')}} </h3>
                    ...
                    <div class="kt-portlet__head-wrapper">
                        <div class="kt-portlet__head-actions">
                            <div class="dropdown dropdown-inline">
                                <a wire:navigate href="{{ route('admin.menu.create') }}" class="btn btn-brand btn-elevate btn-icon-sm">
    ...
    <div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
        ...
        <div class="kt-portlet kt-portlet--mobile">
            <div class="kt-portlet__head kt-portlet__head-- align-items-center">
                <div class="kt-portlet__head-label">
                    ...
                    <h3 class="kt-portlet__head-title">
                        {{__('validation.massages.list_menu')}}
                    </h3>
                </div>
            ...
            <div class="kt-portlet__body">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="kt_table_1">
                    ...
                    @foreach($menus as $m)
                        <tr>
                            ...
                            <td>
                                @if(app()->getLocale()==='fa')
                                    {{$m->parent ? $m->parent->title : 'بدون والد'}}
                                @else
                                    @if($m->parent != null)
                                        @if($m->parent->title_en != null)
                                            {{$m->parent->title_en}}
                                        @else
                                            {{$m->parent->title}}
                                        @endif
                            ...
                            <td>
                                @switch($m->status)
                                    @case(0)
                                        <div class="badge bg-secondary px-3">{{__('validation.attributes.inactive')}}</div>
                                        @break
                                    @case(1)
                                        <div class="badge bg-success px-3 text-white">{{__('validation.attributes.active')}}</div>
                                        @break
                                @endswitch
                            </td>
                            <td>
                ...
                <div class="mt-2">
                    @if(app()->getLocale() === 'fa')
                        نمایش {{ $menus->firstItem() }} تا {{ $menus->lastItem() }} از {{ $menus->total() }} نتیجه
                    @endif
                </div>
                {{$menus->links()}}
                <!--end: Datatable -->
